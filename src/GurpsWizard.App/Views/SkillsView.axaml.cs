...
public partial class SkillsView : ReactiveUserControl<SkillsViewModel>
{
    ...
    private async void OnSearchPointerMoved(object? sender, PointerEventArgs e)
    {
        ...
        if (ViewModel?.SelectedLibrarySkill is not { } skill) return;

        _isDragging = true;
        try
        {
            var data = new DataObject();
            data.Set("LibrarySkill", skill);
    ...
    {
        if (e.Data.Get("LibrarySkill") is LibrarySkill skill && ViewModel is { } vm)
        {
            vm.SelectedLibrarySkill = skill;
